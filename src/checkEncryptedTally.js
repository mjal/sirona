...
export default function (state) {
  let ballots = keepLastBallotByCredentials(state.ballots)

  const questions = state.setup.payload.election.questions
  let encryptedTally = questions.map((question) => {
  ...
  for (let i = 0; i < ballots.length; i++) {
    for (let j = 0; j < encryptedTally.length; j++) {
      ...
      for (let k = 0; k < encryptedTally[j].length; k++) {
        const alpha =
          ed25519.ExtendedPoint.fromHex(rev(answer.choices[k].alpha))
        const beta  =
        ...
        encryptedTally[j][k].beta =
          encryptedTally[j][k].beta.add(beta)
      }
    }
  }
  ...
  for (let i = 0; i < et.length; i++) {
    for (let j = 0; j < et[i].length; j++) {
      check("encryptedTally", "Encrypted tally microballot correspond to the weighted sum of all ballots",
...
function keepLastBallotByCredentials(ballots) {
  let ballots_by_credential = {}
  for (let i = 0; i < ballots.length; i++) {
    ballots_by_credential[ballots[i].payload.credential]
      = ballots[i]
  }
  return Object.values(ballots_by_credential)
}
