...
program.command("generate-credentials")
  ...
  .action(async function (options) {
    ...
    const privcreds = Object.fromEntries(lines.map((line) => {
    ...
    await fs.promises.writeFile(`${timestamp}.privcreds`, JSON.stringify(privcreds, null, 0));
    await fs.promises.writeFile(`${timestamp}.pubcreds`, JSON.stringify(pubcreds, null, 0));
  });
