export class TarFile {
    constructor(name, mode, uid, gid, size, mtime, checksum, type, linkname, ustar, content) {
        this.name = name;
        this.mode = mode;
        this.uid = uid;
        this.gid = gid;
...
export class TarReader {
    ...
    readFiles() {
        ...
        while (offset < this.data.length) {
            ...
            const content = new TextDecoder().decode(this.data.slice(offset + 512, offset + 512 + size)).replace(/\0/g, '').trim();
            const file = new TarFile(name, mode, uid, gid, size, mtime, checksum, type, linkname, ustar, content);
    ...
    readString(buffer, start, length) {
        return String.fromCharCode(...buffer.subarray(start, start + length)).replace(/\0/g, '').trim();
    }
