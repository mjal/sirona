...
export type t = {
  num_tallied: number,
  total_weight: number,
  encrypted_tally: Array<Ciphertext.Serialized.t[] | Ciphertext.Serialized.t[][]>,
}

export function verify(election: Election.t, encryptedTally: t, ballots: Event.t<Ballot.t>[], credentials: string[]) {
  let talliedBallots = keepLastBallots(ballots);
  const recomputedEncryptedTally = recomputeEncryptedTally(election, talliedBallots, credentials);
  ...
  for (let i = 0; i < election.questions.length; i++) {
    ...
    if (Question.IsQuestionH(question)) {
      for (let j = 0; j < encryptedTally.encrypted_tally[i].length; j++) {
        if (
          Ciphertext.Serialized.toString(encryptedTally.encrypted_tally[i][j]) !==
      ...
      for (let j = 0; j < encryptedTally.encrypted_tally[i].length; j++) {
        for (let k = 0; k < encryptedTally.encrypted_tally[i][j].length; k++) {
          if (
            Ciphertext.Serialized.toString(encryptedTally.encrypted_tally[i][j][k]) !==
            Ciphertext.toString(recomputedEncryptedTally.encrypted_tally[i][j][k])
          ) {
            throw new Error("Encrypted tally is incorrect");
          }
        }
      ...
      const a = encryptedTally.encrypted_tally[i].slice().sort((e1, e2) => e1.alpha > e2.alpha);
      const b = recomputedEncryptedTally.encryptedTally[i].slice().sort((e1, e2) => e1.alpha > e2.alpha);
  ...
  if (recomputedEncryptedTally.total_weight !== Number(encryptedTally.total_weight)) {
...
function recomputeEncryptedTally(election: Election.t, ballots: Event.t<Ballot.t>[], credentials: string[]) {
  ...
  encryptedTally.total_weight = ballots.reduce((acc, ballot) => {
    ...
    const weight = credential.includes(",") ? Number(credential.split(",")[1]) : 1;
    return weight + acc;
  }, 0);
